...
def index():
    content = opener.open(urlMain+"/tv-magazin/").read()
    spl = content.split('<div class="col-xs-8 col-xs-offset-2 col-sm-5 col-md-4 col-sm-offset-0 col-md-offset-0 imagethumbnail ">')
    for i in range(1, len(spl), 1):
        entry = spl[i]
        #thumbnail
        match = re.compile('<img src="([^"]*)" class="pull-left visible-xs" width="212%" style="margin-left:-56%" />', re.DOTALL).findall(entry)
        thumb = match[0]
		#title
...
def playVideo(url):
    try:
        content = opener.open(url+"/play").read()
        match = re.compile('<h3>([^<]*)</h3>', re.DOTALL).findall(content)
        title = match[0]
        #match0=webm, match1=mp4
        match = re.compile('<source type="([^"]*)" src="([^"]*)">', re.DOTALL).findall(content)
        urlFull=match[1][1]
